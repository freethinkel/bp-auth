<script lang="ts">
	...
	let { delay, animationDirection }: Props = $props();
</script>

<div
	class="loader"
	in:transitionWithPrefersReducedMotion={{
		fn: slide,
		duration: 200,
		delay,
		axis: animationDirection === 'horizontal' ? 'x' : 'y'
	}}
	out:transitionWithPrefersReducedMotion={{
		fn: slide,
		duration: 200,
		axis: animationDirection === 'horizontal' ? 'x' : 'y'
	}}
